...
This is the preferred option as it has the easiest migration path (on par with [Option 1](#option-1-contextual-keyword)). It has an advantage over [Option 1](#option-1-contextual-keyword) in that it does not require `body` to become an optional keyword in the long-term, as eventually body will be removed as a contextual keyword. It does, however, risk causing confusion to new users of D as it will have to be explained why _either_ symbol is allowed.
